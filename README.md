...
```
todo-manifest-repository/
├── deployment.yaml    # Deployment (단일 컨테이너: nginx + backend)
...
```yaml
# 변경 전 (플레이스홀더)
image: <ECR_TODO_APP_IMAGE>

# 변경 후 (CI가 자동 업데이트)
image: 123456789012.dkr.ecr.us-west-2.amazonaws.com/todo-app:abc12345
```

> CI 실행 전에 ArgoCD Application을 먼저 생성하면 `<ECR_TODO_APP_IMAGE>` 플레이스홀더 때문에 ImagePullBackOff가 발생합니다.
